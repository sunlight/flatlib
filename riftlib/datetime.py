...
class Datetime:
    ...
    def __init__(self, date, time=0, utcoffset=0, calendar=GREGORIAN, pos=0):
        ...
        elif isinstance(pos, GeoPos):
            self.utcoffset = Time(self.getOffset(pos))
        else:
            self.utcoffset = Time(utcoffset)

        self.datetime = datetime.fromisoformat(self.toString())
    ...
    def getTz(self, pos):
        """ Returns the timezone for a location. """
        return timezone(TimezoneFinder().certain_timezone_at(lng=pos.lon, lat=pos.lat))

    def getOffset(self, pos):
        """ Calculates a timezone offset based on position. """
        tz = self.getTz(pos)
        date = datetime.fromisoformat(self.date.toString() + ' ' + self.time.toString())
        date_local = tz.localize(date)
        date_utc = utc.localize(date)
        return int((date_utc - date_local).total_seconds() / 3600)
